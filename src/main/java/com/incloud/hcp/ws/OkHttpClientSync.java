...
public class OkHttpClientSync {
    ...
    public OkHttpClientSync(String type) {
        ...
        if("COMPRA_REGISTRO".equals(type))
            mOkHttpClient = buildOkHttpClientDocument();
        if("OBTENER_TIPOCAMBIO".equals(type))
            mOkHttpClient = buildOkHttpClientIntegration();
        mOkHttpClient = buildOkHttpClientIntegration();
        if("ORDEN_COMPRA".equals(type))
            mOkHttpClient = buildOkHttpClientIntegration();
        if("SOLICITUD_GUIA".equals(type))
            mOkHttpClient = buildOkHttpClientIntegration();
        if("COMPROBANTE_PAGO".equals(type))
            mOkHttpClient = buildOkHttpClientDocument();
        if("DOCUMENTO_REGISTRO".equals(type))
            mOkHttpClient = buildOkHttpClientDocument();
        if("CONDICION_PAGO".equals(type))
            mOkHttpClient = buildOkHttpClientDocument();
        if("COMPROBANTE_RETENCION".equals(type))
            mOkHttpClient = buildOkHttpClientDocument();
        if("DOCUMENTO_ANULADO".equals(type))
            mOkHttpClient = buildOkHttpClientDocument();
...
*/
    ...
    private OkHttpClient buildOkHttpClientDocument() {
        OkHttpClient.Builder builder = new OkHttpClient.Builder()
                .connectTimeout(5, TimeUnit.SECONDS)
